...
exports.snipImage_GLL = function (res,config,outPutPath,t_l_lat,t_l_lon,b_r_lat,b_r_lon,dateTime,fileType, next) {
    ...
    glob(tmpBasePath + filterKey, function (err, files) {
        ...
        perBlockHeight = config.Height;
        perBlockWidth = config.Width;
        perLatLines = perBlockHeight / config.LatScope;
        perLonLines = perBlockWidth / config.LonScope;
        ...
        if (shotFiles.length == 0) {
            ...
            shotFiles.forEach(function (item) {
                    var buf = fs.readFileSync(item);
                    var srcImg = new Image;
                    srcImg.src = buf;
                    if (tmpWidth + item.width <= cutWidth) {
                        if (!isErr) {
                            ctx.drawImage(srcImg, tmpWidth * -1, tmpHeight * -1, item.width, item.height);
                        }
                        tmpWidth += item.width;
                    }
                    else {
...
function ItemModel() {
    ...
    self.check = function (l_t_lat,l_t_lon,r_b_lat,r_b_lon) {
        ...
        // if((block_r_b_lat >= l_t_lat && block_r_b_lon <= l_t_lon) ||  //10°块右下角经纬度小于截图左上角
        //     (block_l_t_lat <= r_b_lat && block_l_t_lon >= r_b_lon) || //10°块左上角经纬度大于截图右下角
        //     (block_l_t_lat <= r_b_lat && block_r_b_lon <= l_t_lon) || //10°块右上角经纬度小于截图左下角
        //     (block_r_b_lat >= l_t_lat && block_l_t_lon >= r_b_lon)){  //10°块左下下角经纬度大于截图右上角
        //     return false;
        // }
        //该10°块文件完全包含在截图范围内
        if(block_l_t_lat <= l_t_lat && block_l_t_lon >=l_t_lon && block_r_b_lat >= r_b_lat && block_r_b_lon <= r_b_lon) {
            ...
            //10°块经度范围包含了右下角经度,说明该10°块文件在截图区域的右边线上
                ...
                self.iOffset = (r_b_lon - block_l_t_lon) * perLonLines;
                self.width = perBlockWidth - self.iOffset;
